...
# INNER JOIN 
def inner_join(conn, tableName1, tableName2, common_column):
    cur = conn.cursor()
    left_join_statement = f""" SELECT * FROM {tableName1}
                                INNER JOIN {tableName2}
                                ON {tableName1}.{common_column} = {tableName2}.{common_column}
    ;"""
    cur.execute(left_join_statement)
    results = cur.fetchall()
    for row in results:
        print(row)
    conn.commit()
...
# LEFT JOIN
def left_join(conn, tableName1, tableName2, common_column):
    cur = conn.cursor()
    left_join_statement = f""" SELECT * FROM {tableName1}
...
values_1 = [(1,"Selena Gomez"),(2,"Adam Ray"),(3,"Shawn Mendes"),(4,"Justin Bieber"),(5,"Bella Hadid"),(6,"Random1"),(7,"Random2")]
...
# INNER JOIN USAGE
print('\nDetails of customers WITH EXISTING BANK BRANCHES:\n')
inner_join(conn, table_1, table_2, 'customer_id')

# LEFT JOIN USAGE
print('\nDetails of ALL customers along with bank branch details:\n')
left_join(conn, table_1, table_2, 'customer_id')
